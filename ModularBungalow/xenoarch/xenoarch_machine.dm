/obj/machinery/xenoarch
	icon = 'ModularBungalow/xenoarch/icons/xenoarch_machines.dmi'
	density = TRUE
...
/obj/machinery/xenoarch/proc/insert_xeno_item(obj/item/insert_item, mob/living/user)
	...
	to_chat(user, "The [insert_item] has been inserted into [src].")
...
/obj/machinery/xenoarch/examine(mob/user)
	...
	if(holding_storage?.contents.len)
		...
		to_chat(user, "There [is_singular ? "is" : "are"] [holding_storage.contents.len] [is_singular ? "item" : "items"] currently in the buffer.")
		to_chat(user, "You can remove the contents, but the machine is currently trying to process the items!")
...
/obj/machinery/xenoarch/researcher/examine(mob/user)
	. = ..()
	to_chat(user, "[current_research]/100 research points. Research more xenoarchaeological items.")
...
/obj/machinery/xenoarch/researcher/attackby(obj/item/weapon, mob/user, params)
	if(istype(weapon, /obj/item/storage/bag/xenoarch))
		...
		for(var/check_item in xenoarch_bag.contents)
			...
			to_chat(user, "The strange rock has been inserted into [src].")
		world_compare = world.time + process_speed
		addtimer(CALLBACK(src, PROC_REF(do_machine_process)), process_speed)
		return
...
/obj/machinery/xenoarch/scanner/attackby(obj/item/weapon, mob/user, params)
	...
	if(istype(weapon, /obj/item/xenoarch/strange_rock))
		...
		if(!do_after(user, scan_speed, target = src))
			to_chat(user, "You interrupt the scanning process, aborting process.")
			return
		if(strange_rock.get_scanned())
			to_chat(user, "You successfully scan the strange rock. It will now report its depth in real time!")
			return
		to_chat(user, "The strange rock was unable to be scanned, perhaps it has already been scanned?")
		return
	return ..()

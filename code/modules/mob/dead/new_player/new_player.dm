	...
	for(var/category in department_categories)
		...
		for(var/job in GLOB.position_categories[category]["jobs"])
			...
			if(job_datum && IsJobUnavailable(job_datum.title, TRUE) == JOB_AVAILABLE)
				var/altjobline = "" //bungalow edit - alt job titles
				var/command_bold = ""
				if(client && client.prefs && client.prefs.alt_titles_preferences[job_datum.title])//bungalow edit - alt job titles
					altjobline = "(as [client.prefs.alt_titles_preferences[job_datum.title]])"//bungalow edit - alt job titles

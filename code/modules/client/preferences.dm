...
/datum/preferences
	...
	var/list/alt_titles_preferences = list()//bungalow edit - alt job titles
		...
		if (1) // Game Preferences
			...
			for (var/special_role in GLOB.special_roles)
				if(is_banned_from(user.ckey, special_role))
					...
					if(ispath(GLOB.special_roles[special_role]) && CONFIG_GET(flag/use_age_restriction_for_jobs)) //If it's a game mode antag, check if the player meets the minimum age
						var/mode_path = GLOB.special_roles[special_role]
...
/datum/preferences/proc/SetChoices(mob/user, limit = 17, list/splitJobs = list("Chief Engineer"), widthPerColumn = 295, height = 620)
	...
	if(SSjob.occupations.len <= 0)
		...
		for(var/datum/job/job in sortList(SSjob.occupations, GLOBAL_PROC_REF(cmp_job_display_asc)))
			...
			var/rank = job.title
			var/displayed_rank = rank//bungalow edit - alt job titles
			if(job.alt_titles.len && (rank in alt_titles_preferences))
				displayed_rank = alt_titles_preferences[rank]//bungalow end
			lastJob = job
			...
			if((job_preferences[SSjob.overflow_role] == JP_LOW) && (rank != SSjob.overflow_role) && !is_banned_from(user.ckey, SSjob.overflow_role))
				...
				continue
			var/rank_title_line = "[displayed_rank]" //bungalow edit-alt job titles
	...
	if(href_list["preference"] == "job")
		switch(href_list["task"])
			...
			if("random")
				...
				SetChoices(user)//bungalow edit alt job titles
			if("alt_title")
				...
				if(user.client.prefs.exp[job_title] >= CONFIG_GET(number/ultra_senior_timelock))//100 hours, i can't be fucked making this a config
					...
					for(var/possible_job_titles in J.senior_title)
						titles_list += possible_job_titles
				...
				if(chosen_title)
					if(chosen_title == job_title)
						...
						alt_titles_preferences[job_title] = chosen_title//bungalow end
				SetChoices(user)
			if("setJobLevel")

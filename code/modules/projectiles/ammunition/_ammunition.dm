...
/obj/item/ammo_casing/proc/bounce_away(still_warm = FALSE, bounce_delay = 3)
	...
	if(still_warm && T?.bullet_sizzle)
		...
		addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(playsound), src, T.bullet_bounce_sound, 20, 1), bounce_delay) //Soft / non-solid turfs that shouldn't make a sound when a shell casing is ejected over them.
